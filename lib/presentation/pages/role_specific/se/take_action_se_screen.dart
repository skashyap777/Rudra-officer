...
class TakeActionSeScreen extends ConsumerStatefulWidget {
  final String caseId;
  final String reportId;

  const TakeActionSeScreen({
    super.key,
  ...
  @override
  ConsumerState<TakeActionSeScreen> createState() => _TakeActionSeScreenState();
}

class _TakeActionSeScreenState extends ConsumerState<TakeActionSeScreen> {
  ...
  Future<void> _showApproveConfirm() async {
    ...
    if (confirmed == true) _doApproveCase();
  }
  ...
  Future<void> _showReassignConfirm() async {
    ...
    final confirmed = await _showConfirmDialog(
      title: 'Reassign to EE',
      subtitle: 'Are you sure you want to reassign this case to EE?',
      reportId: widget.reportId,
      actionLabel: 'Reassign',
      actionColor: _yellow,
    );
    if (confirmed == true) _doReassignToEe();
  }
  ...
  Future<void> _doReassignToEe() async {
    setState(() => _isReassignLoading = true);
    try {
      final repo = await ref.read(reportRepositoryProvider.future);
      await repo.reassignToEe(
        caseId: widget.caseId,
        otherReason: _otherRemarksController.text.trim(),
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Report re-assigned to EE successfully')),
        );
        context.pop();
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Action Panel (SE)', style: TextStyle(fontFamily: 'inter_medium', fontSize: 16, color: Colors.white)),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          'Are you satisfied with the report? You can approve it or request a reassignment to Executive Engineer.',
                        ...
                        const SizedBox(height: 12),
                        _RadioOption(
                          label: 'Unsatisfied (Return to EE)',
                ...
                if (_isSatisfied == false)
                  ElevatedButton(
                    ...
                    child: _isReassignLoading
                        ...
                        : const Text('RETURN TO EE', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
                  ),
                if (_isSatisfied == true)
                  ElevatedButton(
                    onPressed: (_canApprove && !_isForwardLoading) ? _showApproveConfirm : null,
                    style: ElevatedButton.styleFrom(
                      minimumSize: const Size.fromHeight(60),
                    ...
                    child: _isForwardLoading
                        ? const SizedBox(height: 24, width: 24, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 3))
                        : const Text('APPROVE CASE', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
                  ),
